...
*April 2026 · Confidential · Share with lead developer only*
...
export function generateArticleJsonLd(article: any): object[] {
  ...
  schemas.push({
    ...
    "dateModified": article.contentUpdateLog?.[0]?.updateDate
      ?? article.lastVerifiedDate ?? article.updatedAt,
    "author": {
      "@type": "Person",
...
export async function GET() {
  ...
  const articleLines = articles
    .map((article: any) => {
      const url =
        `https://www.caseport.io/insights/${article.slug}`
...
---
*End of developer task sheet · CasePort · April 2026*