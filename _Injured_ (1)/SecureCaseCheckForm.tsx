...
export default function SecureCaseCheckForm({ isOpen, onClose }: Props) {
  ...
  const handleNext = () => {
    if (currentScreen === 8 && formData.hasLawyer === 'yes') {
      setCurrentScreen(14) // Hard stop
      return
    }
    if (currentScreen === 11 && formData.hasDocuments !== 'Upload documents now') {
    ...
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-end md:items-center justify-center">
      <div className="bg-[#0F1419] w-full max-w-xl md:rounded-2xl md:border md:border-white/[0.06] max-h-[95vh] md:max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-6 md:px-8 py-8">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-3">
                {[
                  ...
                  <button
                    key={care}
                    onClick={() => handleInputChange('medicalCare', care)}
                    className={`p-4 text-left rounded-xl transition-all duration-300 ${
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <button
                    key={option}
                    onClick={() => handleInputChange('fault', option)}
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-3">
                {['Yes', 'Not right now', 'Text is better'].map((option) => (
                  <button
                    key={option}
                    onClick={() => handleInputChange('canTalkNow', option)}
                    className={`p-4 text-left rounded-xl transition-all duration-300 ${
          ...
          {currentScreen === 12 && formData.hasDocuments === 'Upload documents now' && (
            <div className="space-y-6">
              ...
              <div className="border-2 border-dashed border-white/[0.1] rounded-xl p-10 text-center hover:border-[#C9A96E]/30 transition-colors duration-300">
        ...
        <div className="border-t border-white/[0.06] px-6 md:px-8 py-5 flex gap-3">
          ...
          <button
            onClick={handleNext}
            className="flex-1 flex items-center justify-center gap-2 px-5 py-3 bg-[#C9A96E] hover:bg-[#B8985D] text-[#0F1419] text-sm font-semibold rounded-xl transition-all duration-300"
          >
            <span>
              {currentScreen === totalScreens ? 'Complete Secure Case Check' : 'Continue'}
            </span>
            {currentScreen < totalScreens && <ChevronRight size={16} />}
          </button>
        </div>
