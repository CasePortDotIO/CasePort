/**
 * The domain service layer (Section 4). All business logic lives behind these
 * pure services. Routes, Payload hooks, and Inngest functions call services;
 * they never reach into the database directly for business operations. Each
 * service is designed to become an MCP tool: clean inputs, clean outputs, no
 * hidden side channels.
 *
 * Phase 0 stands up ComplianceService (the wall) and the RoutingService
 * signature (W1). The remaining services are typed skeletons whose bodies are
 * filled in their phases:
 *   IntakeService, QualificationService, DossierService  Phase 1
 *   WalletService                                        Phase 2 and 3
 *   OutcomeService, IntelligenceService                  Phase 4
 */
export { ComplianceService } from './ComplianceService'
export { RoutingService, decideGeographicRoute, createRoutingService } from './RoutingService'
export type { RoutingIntake, GeographicRoutingDecision, RoutingServiceInstance } from './RoutingService'
export { createDeliveryService } from './DeliveryService'
export type { DeliveryService, DeliveryOutcome } from './DeliveryService'
export type * from './deliveryPorts'
export { createIntakeService } from './IntakeService'
export type { IntakeService } from './IntakeService'
export type * from './ports'
export { QualificationService } from './QualificationService'
export { DossierService } from './DossierService'
export { createWalletService } from './WalletService'
export type { WalletService } from './WalletService'
export type * from './walletPorts'
export { createGlassBoxService } from './GlassBoxService'
export type { GlassBoxService } from './GlassBoxService'
export { createOutcomeService } from './OutcomeService'
export type { OutcomeService } from './OutcomeService'
export { createIntelligenceService, nextModelVersion } from './IntelligenceService'
export type { IntelligenceService } from './IntelligenceService'
export * from './scps'
export type * from './intelligencePorts'
export { createAgentService, slaStatus } from './AgentService'
export type { AgentService } from './AgentService'
export type * from './agentPorts'
export { createIntelligenceCoreService } from './IntelligenceCoreService'
export type { IntelligenceCoreService } from './IntelligenceCoreService'
export type * from './intelligenceCorePorts'
export { createDemandCaptureService } from './DemandCaptureService'
export type { DemandCaptureService } from './DemandCaptureService'
export type * from './demandCapturePorts'
export { createIngestionService } from './IngestionService'
export type { IngestionService } from './IngestionService'
export type * from './ingestionPorts'
export { createDemandCaptureAgentService } from './DemandCaptureAgentService'
export type { DemandCaptureAgentService } from './DemandCaptureAgentService'
export type * from './demandAgentPorts'
export { createSynthesisService } from './SynthesisService'
export type { SynthesisService } from './SynthesisService'
export type * from './synthesisPorts'
export { createB2BCaptureService } from './B2BCaptureService'
export type { B2BCaptureService } from './B2BCaptureService'
export type * from './b2bPorts'
export { createRecommendationLoopService } from './RecommendationLoopService'
export type { RecommendationLoopService } from './RecommendationLoopService'
export type * from './recommendationLoopPorts'
export { createLearningLoopService, surfaceBoosts } from './LearningLoopService'
export type { LearningLoopService } from './LearningLoopService'
export type * from './learningPorts'
export { createBriefingService, renderBriefing } from './BriefingService'
export type { BriefingService } from './BriefingService'
export type * from './briefingPorts'
export { createPromotionService } from './PromotionService'
export type { PromotionService } from './PromotionService'
export type * from './promotionPorts'
