...
import { SCREENS, PHASE_MAP, PHASES, type ScreenConfig } from '@/lib/formData';
...
export function useFormState() {
  ...
  const totalScreens = SCREENS.length;
    ...
    // Calculate progress aligned to exactly match Phase dots
    const totalPhases = PHASES.length;
    const phaseInterval = 100 / (totalPhases - 1);
    const phaseBase = (currentPhaseIndex / (totalPhases - 1)) * 100;
    
    const screensInPhase = SCREENS.filter(s => PHASE_MAP[s.id] === currentPhaseIndex);
    const screenIndexInPhase = screensInPhase.findIndex(s => s.id === state.currentScreen);
    
    let progressPercent = phaseBase + (screenIndexInPhase / Math.max(1, screensInPhase.length)) * phaseInterval;
  ...
  const dismissSoftFail = useCallback(() => {
    ...
    const dismissHardStop = useCallback(() => {
      setState(prev => {
        const nextAnswers = { ...prev.answers };
        delete nextAnswers[prev.currentScreen];
  ...
  const submitApplication = useCallback(async () => {
    ...
    try {
      // Convert to human-readable questions and answers
        const stringAnswers: Record<string, string | string[]> = {};
        Object.entries(answers).forEach(([k, v]) => {
          const screenId = parseInt(k, 10);
          const screen = SCREENS.find(s => s.id === screenId);
          if (!screen) return;
          
          const questionKey = screen.question || screen.headline;
          let humanValue = v;
          
          // Map single choice IDs to the actual labels
          if (screen.type === 'single-choice' && Array.isArray(screen.options)) {
            const matchedOption = screen.options.find(opt => opt.id === v);
            if (matchedOption) {
              humanValue = matchedOption.label;
            }
          }
          
          stringAnswers[questionKey] = humanValue as string | string[];
        });
