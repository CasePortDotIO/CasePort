    ...
    <div ref={ref} id="roi-calculator" className="mx-auto max-w-6xl px-5 sm:px-6 lg:px-8">
      ...
      >
        ...
        <p className="mt-5 max-w-2xl mx-auto text-[1.0625rem] text-[#B0B8C4] leading-[1.75]">
          Adjust the inputs below to model your potential return. These projections are illustrative
          and based on the parameters you provide.
        </p>
      </motion.div>
      ...
      >
        ...
        <div className="rounded-2xl border border-amber-500/15 bg-gradient-to-b from-white/[0.05] to-white/[0.02] p-7 sm:p-8 backdrop-blur-xl">
          ...
          <div className="space-y-9">
            ...
            />
            <SliderInput
              label="Average Case Settlement Value"
              value={avgCaseValue}
              onChange={setAvgCaseValue}
              min={10000}
              max={500000}
              step={5000}
              display={formatCurrency(avgCaseValue)}
              color="amber"
            />
            <SliderInput
        ...
        {/* Results Panel */}
        <div
          className="rounded-2xl border border-amber-500/20 p-7 sm:p-8 backdrop-blur-xl glow-gold"
          style={{
            background:
              'linear-gradient(180deg, rgba(245,158,11,0.08) 0%, rgba(251,191,36,0.03) 50%, rgba(255,255,255,0.02) 100%)',
          }}
        >
          ...
          <div className="space-y-5">
            <ResultRow
              ...
              value={`${calculations.signedCases}`}
              highlight
            />
            <ResultRow
            ...
            <ResultRow
              ...
              value={formatCurrency(calculations.monthlyCost)}
            />
            <ResultRow
            ...
            <ResultRow
              ...
              value={formatCurrency(calculations.annualRevenue)}
              highlight
              large
            />
            <ResultRow
          ...
          <div className="mt-7 pt-5 border-t border-amber-500/10">
            <p className="text-[11px] text-[#6B7280] leading-relaxed">
              * These figures are illustrative projections only and do not constitute a guarantee of
              results. Actual outcomes depend on market conditions, case quality, firm capacity, and
              other factors. CasePort does not guarantee any specific number of leads, cases, or
              revenue.
            </p>
          </div>
        </div>
...
function ResultRow({
  ...
  return (
    <div className="flex items-baseline justify-between">
      <span className={`text-[14px] ${highlight ? 'text-[#D1D5DB]' : 'text-[#6B7280]'}`}>
        {label}
      </span>
      <span
        className={`font-mono tabular-nums font-semibold ${large ? 'text-xl sm:text-2xl' : 'text-lg'} ${highlight ? 'text-white' : 'text-[#9CA3AF]'}`}
      >
        {value}
      </span>
    </div>
