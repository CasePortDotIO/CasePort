      ...
      <div className="container-5">
        ...
        <div className="grid grid-4">
          {articles.map((article) => (
            <Link key={article.id} href={`/accidents/${categorySlug || article.guideCategory?.slug || 'accident'}/${article.slug}`} className="card link r">
...
function CategoryBlockRenderer({ block, updatedAt, categorySlug, categoryName, isMedical, proseSectionsLen, articleUrl }: { block: CategoryBlock; updatedAt?: string; categorySlug?: string; categoryName?: string; isMedical?: boolean; proseSectionsLen?: number; articleUrl?: string }) {
  switch (block.blockType) {
    case 'accidentCategoryDirectAnswer': return <DirectAnswerBlock block={block} updatedAt={updatedAt} />
    case 'accidentCategoryQuickAnswerStats': return <QuickAnswerStatsBlock block={block} />
    case 'accidentCategoryKeyTakeaways': return <KeyTakeawaysBlock block={block} />
    case 'accidentCategoryProseSections': return <ProseSectionsBlock block={block} />
    case 'accidentCategoryFAQ': return <FAQBlock block={block} />
    case 'accidentCategoryStatuteDeadlines': return <StatuteDeadlinesBlock block={block} />
    case 'accidentCategoryWhyImportant': return <WhyImportantBlock block={block} />
    case 'accidentCategorySectionTOC': return <SectionTOC />
    case 'accidentArticleDirectAnswer': return <ArticleDirectAnswerBlock block={block} updatedAt={updatedAt} />
    case 'accidentArticleKeyTakeaways': return <ArticleKeyTakeawaysBlock block={block} />
    case 'accidentArticleFAQ': return <ArticleFAQBlock block={block} />
    case 'accidentArticleRelatedGuides': return <ArticleRelatedGuidesBlock block={block} categorySlug={categorySlug} />
    case 'accidentArticleSources': return <ArticleSourcesBlock block={block} categorySlug={categorySlug} categoryName={categoryName} />
    case 'accidentArticleTimelineSteps': return <ArticleTimelineStepsBlock block={block} />
    case 'accidentArticleSettlementTable': return <ArticleSettlementTableBlock block={block} />
    case 'accidentArticleProseContent': return <ArticleProseContentBlock block={block} />
    case 'accidentArticleStatuteBars': return <ArticleStatuteBarsBlock block={block} />
    case 'accidentArticleExpert': return <ArticleExpertBlock block={block} />
    case 'accidentArticleCTA': return <ArticleCTABlock block={block} />
    case 'accidentArticleTakeHome': return <ArticleTakeHomeBlock />
    case 'accidentCategoryRelatedGuides': {
      const articles = (block.articles as any[]) || []
      if (!articles.length) return null
      return (
        <section className="section bg-white">
          <div className="container-5">
            ...
            <div className="grid grid-4">
              {articles.map((article) => (
                <Link key={article.id} href={`/accidents/${article.guideCategory?.slug || categorySlug}/${article.slug}`} className="card link r">
                  <h3 style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{article.title}</h3>
                  {article.excerpt && (
                    <p style={{ fontSize: '.85rem', color: 'var(--muted)', marginTop: '.4rem', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{article.excerpt}</p>
                  )}
    ...
    case 'accidentCategoryTakeHome': return <TakeHome bg="bg-warm" />
    case 'accidentCategoryHowWeKeepAccurate': {
    ...
    case 'accidentCategoryExploreMore': {
          ...
          <div className="container-5">
            ...
            <div className="grid grid-4">
              {selectedCategories.map((cat) => (
                <Link key={cat.id} href={`/accidents/${cat.slug}`} className="card link r">
...
export function AccidentCategoryClient({ category }: { category: Category }) {
  ...
  const articleUrl = `/accidents/${slug}`

  // Extract reading time from prose sections block if present
  const directAnswerBlock = blocks.find((b) => b.blockType === 'accidentCategoryDirectAnswer')
  const directAnswer = directAnswerBlock?.text || ''
  const proseBlock = blocks.find((b) => b.blockType === 'accidentCategoryProseSections')
  ...
  const faqs = blocks.filter((b) => b.blockType === 'accidentCategoryFAQ').flatMap((b) => (b.items || []).map((f: any) => ({ q: f.question, a: f.answer })))

  return (
    <>
      <HeroPhoto
        crumbs={<Breadcrumbs items={[{ label: 'Home', href: '/' }, { label: 'Accidents', href: '/accidents' }, { label: category.title, href: `/accidents/${slug}` }]} />}
      ...
      <JsonLd
        data={[
          ...
          breadcrumb([
            ...
            { name: category.title, url: `/accidents/${slug}` },
          ]),
