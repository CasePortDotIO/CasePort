...
import { accidentTypes, cityAccidentTypes, stateLawFor, type City } from '@/data'
...
import { article, breadcrumb, faqSchema, howto, orgGraph, speakable } from '@/lib/accidents-schema'
...
const CITY_SPECIALS: [string, string, string, string][] = [
  [
    'what-to-do-after',
    'doc',
    'What to Do After an Accident in',
  ...
  [
    ...
    'Where {city} crashes cluster — local crash data.',
  ],
]
...
export function CityPage({
  ...
  const cFaqs = [
    ...
    {
      q: `What negligence rule applies to a ${city.name} accident?`,
      a: law.fault_rules.direct_answer,
    },
    {
        ...
        <div className="container-4">
          ...
          <div className="law-callout">
            <div className="law-box sage">
              ...
              <Link href={`/accidents/${r.slug}/fault-rules`}>Learn more</Link>
            </div>
            <div className="law-box terra">
              ...
              <Link href={`/accidents/${r.slug}/statute-of-limitations`}>Learn more</Link>
            </div>
          </div>
        </div>
      </section>
            ...
            {cityAccidentTypes.map((at) => {
              ...
              return (
                <Link
                  ...
                  style={
                    active
                      ? { borderColor: 'var(--sage)', background: 'rgba(74,140,126,0.05)' }
                      : undefined
                  }
                >
        ...
        data={[
          ...
          breadcrumb([
            ...
            ...crumbItems.map((ci, i) => ({
              name: ci.label,
              url:
                ci.href ?? `/accidents/${r.cityKey}/${city.slug}${typeSlug ? '/' + typeSlug : ''}`,
            })),
          ]),
          howto({
