...
import { guideScene } from '@/lib/accidents-guide'
import {
  article,
  breadcrumb,
  faqSchema,
...
function spokeContent(
  slug: string,
  spokeSlug: string,
  name: string,
  lc: string,
  avg: number,
): { meta: SpokeMeta; body: React.ReactNode } {
  if (spokeSlug === 'what-to-do') {
    return {
      meta: {
        ...
        faq: [
          {
            q: `What is the first thing to do after a ${lc}?`,
            a: `Ensure everyone is safe and call 911 — request police even for a minor incident. The official report is the single most important document in your claim. Then, if it is safe, photograph the scene before anything is moved.`,
          },
          {
            q: `Should I see a doctor after a ${lc} even if I feel fine?`,
          ...
          {
            q: `What should I not do after a ${lc}?`,
            a: `Do not admit fault, apologize, or say "I’m fine"; do not give the other side’s insurer a recorded statement; and do not accept a quick settlement before you know the full extent of your injuries.`,
          },
        ],
      },
      body: (
        <section className="section bg-white">
          <div className="container-4">
            ...
            <p className="note" style={{ marginTop: '1.5rem' }}>
              <Icon name="alertC" />
              <span>
                If anyone is seriously hurt, call 911 first. This is general guidance, not legal or
                medical advice.
              </span>
            </p>
          </div>
  ...
  if (spokeSlug === 'settlement-amounts') {
    ...
    return {
      meta: {
        ...
        faq: [
          {
            q: `How much is the average ${lc} settlement?`,
            a: `There is no reliable "average" because value depends on severity, liability, and state law. Minor injuries may settle in the low five figures; severe and catastrophic cases reach six and seven figures. The multiplier method (economic damages × 1.5x–5x) drives the non-economic portion.`,
          },
          {
            q: `How is a ${lc} settlement calculated?`,
            a: `Adjusters total your economic damages (medical bills, lost wages) and multiply by roughly 1.5x to 5x based on severity to estimate pain and suffering. Your state’s negligence rule is then applied, reducing or barring recovery based on your share of fault.`,
          },
          {
            q: `Why is the first settlement offer so low?`,
            a: `Insurers expect negotiation, so first offers commonly come in 40–60% below real value — often before your full injuries are known. Accepting ends the claim permanently, so it is wise to understand complete value, including future care, first.`,
          },
        ],
      },
      body: (
  ...
  if (spokeSlug === 'do-i-need-a-lawyer') {
    return {
      meta: {
        ...
        faq: [
          {
            q: `Do I need a lawyer for a ${lc}?`,
            a: `Not always, but it usually helps when there are injuries, disputed fault, or a harsh state fault rule. Represented claimants often net more even after fees, because insurers value unrepresented claims lower. Since consultations are free and lawyers work on contingency, asking costs nothing.`,
          },
          {
            q: `How much does a ${lc} lawyer cost?`,
            a: `Most work on contingency: about one-third (33%) of the recovery, often around 40% if the case goes to trial. You pay no upfront fee, and if there is no recovery you generally owe no attorney fee. Case costs are separate.`,
          },
          {
            q: `Is it worth getting a lawyer for a minor ${lc}?`,
            a: `For a truly minor incident with no injuries, clear liability, and a fair offer, you may not need one. But if there is any injury, treatment, or fault dispute, a free consultation is worth it to avoid leaving money on the table.`,
          },
        ],
      },
      body: (
        <section className="section bg-white">
          <div className="container-4">
            <div className="prose">
              ...
              <p>
                Representation tends to pay for itself when injuries are serious or lasting, when
                liability is disputed, when multiple parties or insurers are involved, or when you
                are in a harsh-fault state. Studies and industry data consistently show represented
                claimants net more even after fees, because insurers value unrepresented claims
                lower and use tactics that counsel neutralizes.
              </p>
              <h2>When you might not</h2>
              <p>
                For a truly minor {lc} with no injuries, a clear-liability property-damage-only
                claim, and a fair early offer, you may be able to handle it yourself. The honest
                test: if there is any injury, any treatment, or any fault dispute, a free
                consultation costs nothing and clarifies the stakes.
              </p>
              <h2>It costs nothing to ask</h2>
              <p>
                Personal-injury lawyers work on contingency — no upfront fee, and no attorney fee
                unless they recover for you. A consultation is free and carries no obligation, so
                the downside of asking is zero.
              </p>
            </div>
          </div>
        </section>
  ...
  return {
    meta: {
      ...
      faq: [
        {
          q: `How long do I have to file a ${lc} claim?`,
          a: `It depends on your state — typically 1 to 6 years from the date of injury, most commonly 2 to 3. The deadline is a hard cutoff: miss it and the claim is permanently barred. Check your state’s page for the exact figure.`,
        },
        {
          q: `What happens if I miss the deadline to file?`,
          a: `If you file after the statute of limitations expires, the court will dismiss the case and you lose the right to recover, regardless of how clear the liability is. A few narrow exceptions exist, but you cannot rely on them.`,
        },
        {
          q: `When does the filing clock start?`,
          a: `Usually on the date of the injury. Some states use a "discovery rule" that starts it when the harm was discovered, and claims involving minors may be paused until adulthood. Government claims often require notice within months.`,
        },
      ],
    },
    body: (
...
export function guideSpokeMeta(pillarSlug: string, spokeSlug: string) {
  ...
  const at = accidentTypes[pillarSlug]
  const avg =
    at?.stats?.[0]?.value && /(\d+)/.exec(at.stats[0].value)
      ? parseInt(/(\d+)/.exec(at.stats[0].value)![1], 10)
      : 50
  ...
  return {
    ...
    canonical: `/guide/${pillarSlug}/${spokeSlug}`,
  }
}

export function GuideSpokePage({
  pillarSlug,
  spokeSlug,
}: {
  pillarSlug: string
  ...
  return (
    <>
      <HeroPhoto
        crumbs={
          <Breadcrumbs
            items={[
              ...
              { label: name, href: `/guide/${pillarSlug}` },
              { label: spoke.label },
            ]}
          />
        }
        ...
        byline={
          <Byline
            ...
            isMedical={isMedical}
            minutes={readingMinutes(meta.lead)}
            onDark
          />
        }
      />
      ...
      <Sources
        medical={isMedical}
        citeTitle={meta.title}
        citeUrl={`/guide/${pillarSlug}/${spokeSlug}`}
      />
      ...
      <JsonLd
        data={[
          ...
          ...(spokeSlug === 'what-to-do'
            ? [
                howto({
                  name: `What to do after a ${lc}`,
                  description: `Step-by-step actions to protect a ${lc} claim.`,
