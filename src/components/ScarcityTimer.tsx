...
export default function ScarcityTimer({ market }: ScarcityTimerProps) {
  ...
  if (market.status === 'capped') {
    return (
      ...
      >
        ...
        >
          WAITLIST #
          {(market.id.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) % 50) + 1}
        </span>
      </div>
    )
  }
  ...
  if (market.status === 'limited') {
    return (
      <div
        className="flex items-center gap-2 px-3 py-2 rounded-[8px] animate-pulse"
        style={{ background: 'rgba(245, 158, 11, 0.15)' }}
      >
        <Clock size={14} className="text-[#F59E0B]" />
        <span
          className="text-[12px] text-[#F59E0B] font-bold"
          style={{ fontFamily: "'JetBrains Mono', monospace" }}
        >
  ...
  return (
    ...
    >
      ...
      <span
        className="text-[12px] text-[#22D3EE]"
        style={{ fontFamily: "'JetBrains Mono', monospace" }}
      >
