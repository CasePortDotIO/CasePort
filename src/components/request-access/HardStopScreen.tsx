...
import { ArrowRight, Bell, CheckCircle2, ShieldX, Users } from 'lucide-react'
...
export function HardStopScreen({ message, support, hardStopReason, onBack }: HardStopScreenProps) {
  ...
  const joinWaitlist = {
    mutate: async (payload: { email: string; firmName: string; hardStopReason?: string }) => {
      ...
      try {
        const res = await fetch('/api/waitlists', {
          ...
          body: JSON.stringify({
            ...
            hardStopReason: payload.hardStopReason,
          }),
        })
      ...
      <header className="px-5 sm:px-8 pt-6 pb-3">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          {onBack && (
            <button
              onClick={onBack}
              className="text-[#0A0E17]/50 hover:text-[#0A0E17]/80 text-[14px] font-medium transition-colors"
              style={{ fontFamily: 'var(--font-geist)' }}
            >
              ← Go back
            </button>
          ...
          >
            ...
            >
              ...
              <p
                ...
                style={{ fontFamily: 'var(--font-geist)' }}
              >
                Eligibility criteria evolve as we expand markets and capacity. We'll notify you
                directly when your firm may qualify.
              </p>
                ...
                {!waitlistJoined ? (
                  ...
                  >
                    <input
                      ...
                      onFocus={(e) => {
                        ;(e.target as HTMLElement).style.borderColor = 'rgba(10, 14, 23, 0.25)'
                      }}
                      onBlur={(e) => {
                        ;(e.target as HTMLElement).style.borderColor = 'rgba(10, 14, 23, 0.1)'
                      }}
                    />
                    <input
                      type="email"
                      ...
                      onBlur={(e) => {
                        ;(e.target as HTMLElement).style.borderColor = waitlistError
                          ? 'rgba(239,68,68,0.4)'
                          : 'rgba(10, 14, 23, 0.1)'
                      }}
                      onKeyDown={(e) => {
                    ...
                    {waitlistError && (
                      <p
                        className="text-[12px] text-red-500/70"
                        style={{ fontFamily: 'var(--font-geist)' }}
                      >
                        {waitlistError}
                      </p>
            ...
            >
              ...
              <p
                ...
                style={{ fontFamily: 'var(--font-geist)' }}
              >
                If you know a PI firm that runs a high-volume, disciplined intake operation, send
                them our way.
              </p>
              <motion.a
                whileHover={{ scale: 1.01 }}
      ...
      <footer className="px-5 sm:px-8 py-5 border-t border-[#0A0E17]/[0.05]">
        <div className="max-w-5xl mx-auto">
          ...
          <p
            ...
            style={{ fontFamily: 'var(--font-geist)' }}
          >
            CasePort is a case acquisition platform, not a law firm. This does not constitute an
            attorney-client relationship. No referral fees paid to or accepted from attorneys. All
            arrangements comply with applicable state bar rules.
          </p>
