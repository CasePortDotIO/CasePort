...
import { NATIONAL, negColor, stateData, stateGrid } from '@/data'
...
function MapPanel({ abbr }: { abbr: string }) {
  ...
  return (
    <>
      ...
      >
        <h3 style={{ fontSize: '1.6rem', fontWeight: 700, color: 'var(--teal)' }}>{s.name}</h3>
        <span
          style={{
            fontSize: '.72rem',
            fontWeight: 700,
      ...
      <div className="map-row">
        <Link href={`/accidents/${stateSlug(abbr)}`} className="card-link">
          See full {s.name} guide
        </Link>
      </div>
      {s.rule === 'pure-contributory' && (
        <div className="map-warn">
          <p>
            Warning: {s.name} uses the harshest negligence standard. Any fault — even 1% — bars your
            entire recovery. What you say in the first hours matters more here than anywhere else.
          </p>
        </div>
      )}
    </>
  ...
  return (
    <div className="empty">
      ...
      <p style={{ fontSize: '.9rem', marginTop: '.4rem' }}>
        Click any state on the map to see your negligence rule, filing deadline, and fault
        threshold.
      </p>
    </div>
  )
      ...
      <div className="container">
        <div className="section-head center">
          ...
          <p className="section-sub center">
            Click your state to see the negligence rule, filing deadline, and fault threshold that
            determines your recovery.
          </p>
        </div>
        <div
        ...
        >
          <div>
            <div className="map-legend">
              <span className="lg">
                <span className="sw" style={{ background: '#c4714a' }}></span>Pure Contributory
                (harshest)
              </span>
              <span className="lg">
                <span className="sw" style={{ background: '#d4a853' }}></span>Modified 50%
              </span>
              <span className="lg">
                <span className="sw" style={{ background: '#4a8c7e' }}></span>Modified 51%
              </span>
              <span className="lg">
                <span className="sw" style={{ background: '#1a4a5a' }}></span>Pure Comparative
                (best)
              </span>
            </div>
