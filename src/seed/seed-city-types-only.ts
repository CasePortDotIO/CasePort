...
async function upsertDoc(payload: any, doc: any): Promise<string> {
...
async function main() {
  ...
  for (const cityKey of CITY_DATA_STATES) {
    ...
    if (!cd) {
      console.log(`   ${cityKey}: no cityData, skipping`)
      continue
    }

    const s = stateData[cd.abbr]
    if (!s) {
      console.log(`   ${cityKey}: no stateData for ${cd.abbr}, skipping`)
      continue
    }
    ...
    for (const city of cd.cities) {
      for (const typeKey of accidentTypeOrder) {
        ...
        const firstHourStepsBlock = firstHourSteps.map((st: any, idx: number) => ({
          stepName: st[0],
            ...
            ? [
                {
                  ...
                  keyFacts: (city.keyFacts || []).map((f: string) => ({ fact: f })),
                },
              ]
            : []),
          {
        ...
        try {
          ...
          grandTotal++
          if (stateTotal % 50 === 0)
            process.stderr.write(`   [${cityKey}] ${stateTotal} pages...\n`)
        } catch (err: any) {
    ...
    console.log(
      `   ✅ ${cd.name}: ${stateTotal} city type pages [${((Date.now() - t0) / 1000).toFixed(1)}s]`,
    )
  }
