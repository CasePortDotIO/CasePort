...
export async function generateMetadata({
  ...
  const { docs } = await payload.find({
    ...
    where: {
      AND: [{ pageType: { equals: 'state' } }, { targetStates: { contains: state.toUpperCase() } }],
    },
    depth: 1,
    limit: 1,
  })
  ...
  return {
    ...
    description:
      ...
      `Learn about personal injury claims, statute of limitations, and settlement ranges in ${stateName}.`,
    alternates: { canonical: `${siteUrl}/guides/states/${state}` },
    openGraph: {
      title: article?.socialHeadline ?? `Personal Injury Guide | ${stateName} | CasePort`,
      description:
        article?.socialDescription ?? `Learn about personal injury claims in ${stateName}.`,
...
export default async function StateGuidePage({ params }: { params: Promise<{ state: string }> }) {
  const { state } = await params
  const payload = await getPayload({ config: configPromise })
  const stateName = STATE_NAMES[state.toUpperCase()] || state
