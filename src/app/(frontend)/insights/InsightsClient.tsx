    ...
    <nav aria-label="Breadcrumb" className="container relative z-10 pt-24 pb-0">
      <ol className="flex items-center gap-2 text-[13px]" itemScope itemType="https://schema.org/BreadcrumbList">
        <li itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">
          <Link href="/" itemProp="item" className="text-cp-text-muted hover:text-cp-cyan transition-colors">
  ...
  return (
    <section className="relative min-h-[100vh] flex flex-col overflow-hidden">
      ...
      <div className="absolute inset-0">
        <img src={HERO_BG} alt="" className="w-full h-full object-cover opacity-30" loading="eager" />
      ...
      <div className="atmo-orb w-[500px] h-[500px] bg-cp-purple/6 top-[15%] right-[-10%]" style={{ animationDelay: "5s" }} />
        ...
        <div className="max-w-[960px]">
          ...
          <Reveal delay={0.2}>
            <p className="mt-10 text-[18px] lg:text-[20px] leading-[1.8] text-cp-text-secondary max-w-[620px]">
              Analysis on personal injury demand, intake performance, search visibility,
              lead economics, and the systems behind better case growth.
            </p>
          </Reveal>
              ...
              <div className="glass-panel overflow-hidden h-full flex flex-col group hover:border-white/[0.12] transition-all duration-500">
                ...
                <div className="p-10 lg:p-12 flex-1 flex flex-col">
                  <div className="flex items-center gap-3 mb-6">
                    <span className="text-[14px] text-cp-text-muted" itemProp="author">By {typeof featured?.author === 'object' ? featured?.author?.name : (featured?.author || 'Editorial Team')}</span>
                    <span className="text-cp-text-muted/40">&middot;</span>
                    <span className="text-[14px] text-cp-text-muted flex items-center gap-1.5"><Clock size={13} /> {featured.readTime}</span>
                    <span className="text-cp-text-muted/40">&middot;</span>
                    <time className="text-[14px] text-cp-text-muted" itemProp="datePublished" dateTime="2026-03-22">{featured.date}</time>
                  </div>
                  ...
                  <p className="mt-6 text-[17px] leading-[1.8] text-cp-text-secondary flex-1" itemProp="description">
                    {featured.excerpt}
                  </p>

                  <div className="mt-8 flex flex-wrap gap-4">
                    {featured?.tags?.map((tag: string) => (
                      <span key={tag} className="flex items-center gap-2 text-[13px] text-cp-text-muted">
                ...
                <Reveal key={article.id} delay={i * 0.1}>
                  <article itemScope itemType="https://schema.org/BlogPosting">
                    <a
                      ...
                      style={{ borderLeft: `3px solid ${getCategoryColor(typeof (typeof article.category === 'object' ? article.category?.title : article.category) === 'object' ? (typeof article.category === 'object' ? article.category?.title : article.category)?.title : (typeof article.category === 'object' ? article.category?.title : article.category))}` }}
                    >
                      {/* Thumbnail */}
                      ...
                      <div className="p-6 lg:p-7 flex flex-col flex-1">
                        <div className="flex items-center justify-between mb-2.5">
                          <span className="system-label text-[10px]" style={{ color: getCategoryColor(typeof (typeof article.category === 'object' ? article.category?.title : article.category) === 'object' ? (typeof article.category === 'object' ? article.category?.title : article.category)?.title : (typeof article.category === 'object' ? article.category?.title : article.category)) }}>
                        ...
                        <p className="mt-2 text-[13px] text-cp-text-muted leading-[1.6] line-clamp-2" itemProp="description">
                          {article.excerpt}
                        </p>
                        <div className="mt-3 flex items-center gap-2">
                          <span className="text-[11px] text-cp-text-muted font-mono">{article.readTime}</span>
...
function EditorialGrid({ fetchedArticles = [] }: { fetchedArticles: any[] }) {
  ...
  return (
    <section id="articles" className="relative py-32 lg:py-44">
      ...
      <div className="atmo-orb w-[600px] h-[600px] bg-cp-purple/5 top-[20%] right-[-15%]" style={{ animationDelay: "3s" }} />
        ...
        <Reveal>
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-10 mb-16">
            ...
            <p className="text-[16px] text-cp-text-muted max-w-[460px] leading-[1.8]">
              Structured for readability by humans, search engines, retrieval systems, and generative engines.
            </p>
          </div>
        </Reveal>
        ...
        <Reveal delay={0.05}>
          <div className="mb-10">
            <div className="relative max-w-[520px]">
              <Search size={18} className="absolute left-5 top-1/2 -translate-y-1/2 text-cp-text-muted" />
              <input
                type="text"
        ...
        {filtered.length === 0 && (
          <Reveal>
            <div className="text-center py-24">
              ...
              <p className="text-[18px] text-cp-text-muted" style={{ fontFamily: "var(--font-display)" }}>
        ...
        <div className="relative h-[180px] overflow-hidden">
          ...
          {article.badge && (
            <div className="absolute top-4 right-4">
              <span className={`system-label text-[9px] px-3 py-1.5 rounded-lg backdrop-blur-sm ${
        ...
        <div className="p-8 lg:p-9 flex flex-col flex-1">
          <div className="flex items-center justify-between mb-5">
            <span
              ...
              style={{ color: getCategoryColor(typeof (typeof article.category === 'object' ? article.category?.title : article.category) === 'object' ? (typeof article.category === 'object' ? article.category?.title : article.category)?.title : (typeof article.category === 'object' ? article.category?.title : article.category)) }}
          ...
          <p className="mt-4 text-[15px] text-cp-text-muted leading-[1.8] line-clamp-2 flex-1" itemProp="description">
            {article.excerpt}
          </p>
      ...
      <div className="container relative z-10">
        <Reveal>
          <div className="relative overflow-hidden rounded-2xl">
            <img src={NEWSLETTER_BG} alt="" className="absolute inset-0 w-full h-full object-cover opacity-40" loading="lazy" />
            ...
            <div className="relative z-10 px-10 py-24 lg:px-24 lg:py-32 text-center">
              ...
              <div className="mt-10 flex items-center justify-center gap-3">
                <div className="flex -space-x-2">
                  {[
                    ...
                    <div key={i} className={`w-8 h-8 rounded-full ${bg} border-2 border-[#0A0E17] flex items-center justify-center`}>
                      <Users size={12} className="text-white/80" />
                    </div>
                  ))}
                </div>
                <span className="text-[14px] text-cp-text-secondary ml-2">
                  Joined by <span className="text-cp-text-primary font-semibold">2,400+</span> PI firm operators
                </span>
              </div>

              {/* Value Pills */}
              <div className="mt-10 flex flex-wrap justify-center gap-4">
                {["Weekly signal summary", "No fluff, just movement", "Built for serious operators", "Free, always"].map((label) => (
                  <span key={label} className="px-5 py-2.5 rounded-lg border border-white/[0.08] bg-white/[0.03] text-[14px] text-cp-text-secondary">
                    {label}
                  </span>
                ))}
              </div>
              ...
              <div className="mt-14 flex flex-col sm:flex-row gap-4 max-w-[500px] mx-auto">
                ...
                />
                <button className="cta-gradient whitespace-nowrap !h-14 !text-[15px]">Subscribe Free</button>
              </div>

              <p className="mt-6 text-[12px] text-cp-text-muted/60">
                No spam. Unsubscribe anytime. Read by managing partners, CMOs, and growth leads at PI firms billing $5M-$50M+.
              </p>
            </div>
          </div>
        </Reveal>
      ...
      <div className="container relative z-10">
        <Reveal>
          ...
          <p className="mt-6 text-[17px] text-cp-text-muted max-w-[540px] leading-[1.8]">
            Organized clusters for focused reading. Each topic is a deep-dive into a specific dimension of case acquisition.
          </p>
        </Reveal>
          ...
          {topicClusters.map((topic, i) => (
            <Reveal key={topic.name} delay={i * 0.08}>
              <div className="topic-card glass-panel p-8 lg:p-10 group cursor-pointer">
                ...
                <p className="mt-4 text-[15px] text-cp-text-muted leading-[1.8]">{topic.description}</p>

                <div className="mt-8 flex items-center justify-between">
                  <span className="system-label text-[10px] text-cp-text-muted">{topic.articleCount} articles</span>
                  <span className="text-[14px] text-cp-cyan flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-[-4px] group-hover:translate-x-0">
        ...
        <Reveal>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-12">
            <div className="glass-panel p-12 lg:p-16">
              <span className="system-label text-cp-text-muted mb-6 block">Editorial Philosophy</span>
              <h2
            ...
            <div className="glass-panel p-12 lg:p-16 flex items-center">
              <div>
                ...
                <p className="text-[18px] lg:text-[20px] leading-[1.85] text-cp-text-secondary font-light">
                  CasePort studies the full path from demand to inquiry to intake to retained case value.
                  We are interested in what improves outcomes, not just activity. That includes search behavior,
                  conversion friction, intake discipline, market economics, and system design.
                </p>
              </div>
            </div>
          </div>
...
function SignalsSection() {
  return (
    <section className="relative py-32 lg:py-44">
      <div className="absolute inset-0 bg-gradient-to-b from-[#0B1120] via-[#0A0E17] to-[#0A0E17]" />
      <div className="atmo-orb w-[500px] h-[500px] bg-cp-cyan/5 bottom-[10%] left-[-10%]" style={{ animationDelay: "7s" }} />
            ...
            <Reveal key={signal.id} delay={i * 0.1}>
              <div className="signal-card glass-panel p-7 lg:p-8 group cursor-pointer">
                <div className="flex items-center justify-between mb-5">
                  ...
                  <span className={`system-label text-[9px] px-2.5 py-1 rounded-full ${
                    signal.priority === "HIGH" ? "text-cp-cyan bg-cp-cyan/10" : "text-cp-text-muted bg-white/[0.04]"
                  }`}>
                    {signal.priority}
                  </span>
                </div>
                ...
                <div className="mt-6 flex items-center justify-between">
                  <div className="flex-1 h-[3px] rounded-full bg-white/[0.06] overflow-hidden mr-4">
                    <div className="signal-bar h-full" style={{ width: `${signal.signalStrength}%` }} />
                  </div>
        ...
        <Reveal>
          <div className="glass-panel max-w-[860px] mx-auto px-12 py-24 lg:px-24 lg:py-32 text-center group hover:border-white/[0.1] transition-all duration-500">
            ...
            <p className="mt-8 text-[17px] text-cp-text-secondary max-w-[540px] mx-auto leading-[1.8]">
              CasePort helps qualified firms turn better intelligence into more controlled case acquisition outcomes.
            </p>
            <div className="mt-12 flex flex-wrap justify-center gap-5">
              <Link href="/" className="cta-gradient flex items-center gap-2.5">
                Learn About CasePort <ArrowRight size={16} />
              </Link>
              <a href="mailto:access@caseport.io" className="cta-secondary flex items-center gap-2.5">
