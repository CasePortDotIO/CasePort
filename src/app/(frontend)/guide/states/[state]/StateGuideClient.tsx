...
interface StateGuideClientProps {
  article: any
  stateName: string
  stateCode: string
  headerNav?: any
...
export default function StateGuideClient({ article, stateName, stateCode }: StateGuideClientProps) {
  const [isMenuOpen, setIsMenuOpen] = useState(false)

  const schemaMarkup = {
    '@context': 'https://schema.org',
    '@type': 'Article',
    headline: `Personal Injury Law in ${stateName}: Statute of Limitations & Settlement Ranges`,
    description: `Complete guide to personal injury law in ${stateName}. Statute of limitations: ${article.statuteOfLimitations?.years || 'N/A'} years. Average settlement: ${article.settlementData?.avg || 'Varies'}.`,
    author: {
      '@type': 'Organization',
      name: 'CasePort.io',
    ...
    keywords: `${stateName} personal injury lawyer, ${stateName} statute of limitations, ${stateName} settlement amounts`,
    inLanguage: 'en-US'
  }
  ...
  const statuteExceptions = article.statuteOfLimitations?.exceptions || []
        ...
        <div className="max-w-4xl mx-auto px-7">
          ...
          <div className="mb-12">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h1 className="text-4xl font-bold text-[#1a4a5a]">{stateName}</h1>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-white rounded-lg p-6 border border-[#e8e2d8]">
              <div>
                ...
                <div className="text-3xl font-bold text-[#1a4a5a]">{article.statuteOfLimitations?.years || 'N/A'} years</div>
                <p className="text-xs text-[#666] mt-2">{article.statuteOfLimitations?.description || 'Varies by case type'}</p>
              </div>
              <div>
                <div className="text-sm text-[#4a8c7e] font-semibold mb-2">Negligence Type</div>
                <div className="text-lg font-bold text-[#1a4a5a]">{article.negligenceType || 'Fault-based'}</div>
              </div>
              <div>
          ...
          {article.statuteOfLimitations && (
            <section className="mb-12">
              <h2 className="text-2xl font-bold text-[#1a4a5a] mb-6">Statute of Limitations in {stateName}</h2>
              <div className="bg-white rounded-lg p-6 border border-[#e8e2d8]">
                <p className="text-[#333] mb-4">
                  {article.statuteOfLimitations.description || `In ${stateName}, the statute of limitations sets the deadline for filing a personal injury lawsuit.`}
                </p>
                {statuteExceptions.length > 0 && (
          ...
          {/* Negligence Section */}
          {article.negligenceDescription && (
            <section className="mb-12">
              <h2 className="text-2xl font-bold text-[#1a4a5a] mb-6">Negligence Laws in {stateName}</h2>
              <div className="bg-white rounded-lg p-6 border border-[#e8e2d8]">
                <h3 className="font-semibold text-[#1a4a5a] mb-3">{article.negligenceType || 'Negligence'}</h3>
                <p className="text-[#333]">{article.negligenceDescription}</p>
              </div>
            </section>
          )}
          ...
          {topCities.length > 0 && (
            <section className="mb-12">
              <h2 className="text-2xl font-bold text-[#1a4a5a] mb-6">Top Cities in {stateName}</h2>
                ...
                {topCities.map((city: string, idx: number) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-semibold text-[#1a4a5a]">{city}, {stateCode}</span>
          ...
          <section className="bg-[#1a4a5a] text-white rounded-lg p-8 mb-12">
            <h2 className="text-2xl font-bold mb-3">Need Legal Help in {stateName}?</h2>
            <p className="mb-6 text-[#e8e2d8]">
              Get a free consultation with a personal injury attorney who knows {stateName} law.
            </p>
