...
const script = import { getPayload } from 'payload';
...
async function run() {
  ...
  for (const article of articles) {
    ...
    let categoryId = categoriesMap.get(article.category);
    if (!categoryId) {
        let titleParam = (typeof article.category === 'object' ? article.category.title : article.category) || 'Insight';
        ...
        categoriesMap.set(article.category, categoryId);
    }
...
fs.writeFileSync('seed-all.ts', script);
