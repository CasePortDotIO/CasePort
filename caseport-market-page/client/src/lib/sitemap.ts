...
export function generateSitemap(baseUrl: string = "https://caseportmp-ktqqzjyn.manus.space"): string {
  const urls = [
    // Main pages
    { loc: `${baseUrl}/`, lastmod: new Date().toISOString().split("T")[0], changefreq: "weekly", priority: "1.0" },
    { loc: `${baseUrl}/markets`, lastmod: new Date().toISOString().split("T")[0], changefreq: "daily", priority: "0.9" },
 ...
 */
export function generateRobotsTxt(baseUrl: string = "https://caseportmp-ktqqzjyn.manus.space"): string {
  return `User-agent: *
Allow: /
Disallow: /admin/
