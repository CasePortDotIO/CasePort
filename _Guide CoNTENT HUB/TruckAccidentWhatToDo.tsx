...
const TruckAccidentWhatToDo = () => {
  ...
  const contentWidth = isMobileView ? 'calc(100% - 32px)' : isTabletView ? '700px' : isDesktopView ? '700px' : '700px';
  ...
  return (
    <div style={{ backgroundColor: '#f9f5ef', minHeight: '100vh' }}>
      ...
      {/* Sticky Header - Exact match to /guides hub */}
      <nav
      ...
      <div style={{ backgroundColor: '#f9f5ef', padding: isMobileView ? '20px' : '24px 40px', borderBottom: '1px solid #e8e2d8' }}>
        <div style={{ display: 'flex', flexDirection: isMobileView ? 'column' : 'row', gap: isMobileView ? '16px' : '32px', flexWrap: 'wrap', fontSize: isMobileView ? '14px' : '13px', color: '#555' }}>
          <div>
            ...
            <div style={{ fontSize: isMobileView ? '13px' : '12px', color: '#999' }}>Personal Injury Attorney | 15+ years experience | Board Certified | $50M+ Recovered</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div style={{ fontSize: isMobileView ? '13px' : '12px', color: '#999' }}>April 28, 2026 • Updated Quarterly</div>
          </div>
          <div>
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <span style={{ backgroundColor: '#c4714a', color: 'white', padding: '6px 10px', borderRadius: '3px', fontSize: isMobileView ? '12px' : '11px', fontWeight: '600' }}>✓ Last Updated: April 28, 2026</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', maxWidth: isMobileView ? '100%' : '1400px', margin: '0 auto', gap: isMobileView ? 0 : '80px', padding: containerPadding, justifyContent: 'center', flexDirection: isMobileView ? 'column' : 'row', alignItems: 'flex-start', width: '100%' }}>
        ...
        <div style={{ flex: isMobileView ? '1 1 100%' : '0 0 auto', width: contentWidth, maxWidth: contentWidth, padding: isMobileView ? '0' : '0', minWidth: 0 }}>
          ...
          <div style={{ marginBottom: '56px', animation: 'fadeIn 0.5s ease' }}>
            <Card style={{ backgroundColor: '#f0f8f6', borderLeft: '4px solid #4a8c7e', padding: '28px', borderRadius: '6px' }}>
              ...
              <p style={{ margin: '12px 0 0 0', color: '#555', lineHeight: '1.8', fontSize: isMobileView ? '18px' : '21px', fontWeight: '500' }}>
          ...
          {/* AI-Optimized TL;DR Section */}
          <div style={{ marginBottom: '56px', animation: 'fadeIn 0.5s ease' }}>
            <Card style={{ backgroundColor: '#fff8f0', borderLeft: '4px solid #c4714a', padding: '28px', borderRadius: '6px' }}>
              <div style={{ fontSize: '12px', fontWeight: '700', color: '#1a4a5a', marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              ...
              <ol style={{ margin: 0, paddingLeft: isMobileView ? '16px' : '20px', color: '#555', fontSize: isMobileView ? '17px' : '20px', lineHeight: '1.8', fontWeight: '500' }}>
                <li><strong>Safety First (0-5 min):</strong> Call 911, move to safe location, turn on hazard lights</li>
                <li><strong>Document Scene (5-15 min):</strong> Photos of damage, license plates, road conditions, witness contact info</li>
                <li><strong>Medical Care (within 24 hrs):</strong> Visit ER or urgent care, get complete evaluation and medical records</li>
                <li><strong>Contact Attorney (within 48 hrs):</strong> Before any settlement talks or insurance statements</li>
                <li><strong>Settlement Reality:</strong> Average <strong>$250,000 with attorney</strong> vs <strong>$50,000 without</strong> (5x difference)</li>
              </ol>
            </Card>
          </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '24px' }}>
                <thead>
                  <tr style={{ backgroundColor: '#1a4a5a', color: 'white' }}>
                    <th style={{ padding: isMobileView ? '12px' : '14px', textAlign: 'left', fontWeight: '600', fontSize: isMobileView ? '15px' : '14px' }}>Factor</th>
                    <th style={{ padding: isMobileView ? '12px' : '14px', textAlign: 'left', fontWeight: '600', fontSize: isMobileView ? '15px' : '14px' }}>With Attorney</th>
                    <th style={{ padding: isMobileView ? '12px' : '14px', textAlign: 'left', fontWeight: '600', fontSize: isMobileView ? '15px' : '14px' }}>Without Attorney</th>
                  </tr>
                </thead>
                <tbody>
                  {[
                    ...
                    <tr key={idx} style={{ backgroundColor: idx % 2 === 0 ? '#f9f5ef' : 'white', borderBottom: '1px solid #e8e2d8' }}>
                      <td style={{ padding: isMobileView ? '12px' : '14px', color: '#555', fontWeight: '600', fontSize: isMobileView ? '15px' : '14px' }}>{row.factor}</td>
                      <td style={{ padding: isMobileView ? '12px' : '14px', color: '#4a8c7e', fontWeight: '600', fontSize: isMobileView ? '15px' : '14px' }}>{row.with}</td>
                      <td style={{ padding: isMobileView ? '12px' : '14px', color: '#999', fontWeight: '500', fontSize: isMobileView ? '15px' : '14px' }}>{row.without}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div id="settlement-ranges" style={{ marginBottom: '56px', scrollMarginTop: '100px' }}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '24px' }}>
                <thead>
                  <tr style={{ backgroundColor: '#1a4a5a', color: 'white' }}>
                    <th style={{ padding: isMobileView ? '12px' : '14px', textAlign: 'left', fontWeight: '600', fontSize: isMobileView ? '13px' : '14px' }}>State</th>
                    <th style={{ padding: isMobileView ? '12px' : '14px', textAlign: 'left', fontWeight: '600', fontSize: isMobileView ? '13px' : '14px' }}>Minor</th>
                    <th style={{ padding: isMobileView ? '12px' : '14px', textAlign: 'left', fontWeight: '600', fontSize: isMobileView ? '13px' : '14px' }}>Moderate</th>
                    <th style={{ padding: isMobileView ? '12px' : '14px', textAlign: 'left', fontWeight: '600', fontSize: isMobileView ? '13px' : '14px' }}>Severe</th>
                    <th style={{ padding: isMobileView ? '12px' : '14px', textAlign: 'left', fontWeight: '600', fontSize: isMobileView ? '13px' : '14px' }}>Catastrophic</th>
                  </tr>
                </thead>
                <tbody>
                  {[
                    ...
                    <tr key={idx} style={{ backgroundColor: idx % 2 === 0 ? '#f9f5ef' : 'white', borderBottom: '1px solid #e8e2d8' }}>
                      <td style={{ padding: isMobileView ? '12px' : '14px', color: '#1a4a5a', fontWeight: '600', fontSize: isMobileView ? '14px' : '14px' }}>{row.state}</td>
                      <td style={{ padding: isMobileView ? '12px' : '14px', color: '#555', fontWeight: '500', fontSize: isMobileView ? '13px' : '14px' }}>{row.minor}</td>
                      <td style={{ padding: isMobileView ? '12px' : '14px', color: '#555', fontWeight: '500', fontSize: isMobileView ? '13px' : '14px' }}>{row.moderate}</td>
                      <td style={{ padding: isMobileView ? '12px' : '14px', color: '#555', fontWeight: '500', fontSize: isMobileView ? '13px' : '14px' }}>{row.severe}</td>
                      <td style={{ padding: isMobileView ? '12px' : '14px', color: '#4a8c7e', fontWeight: '600', fontSize: isMobileView ? '13px' : '14px' }}>{row.catastrophic}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div id="faq" style={{ marginBottom: '56px', scrollMarginTop: '100px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {[
                { q: 'What should I do immediately after a truck accident?', a: 'Move to safety, call 911, document the scene with photos, exchange information with the truck driver (name, phone, insurance, commercial license), get witness contact info, and report to police. Do not admit fault.' },
                { q: 'Do I need an attorney for a truck accident?', a: 'Yes. Attorneys settle cases for 5x more on average ($250,000 vs $50,000). They handle negotiations with insurance companies and maximize your recovery. Most work on contingency (no upfront cost).' },
                ...
                { q: 'What if I was partially at fault?', a: 'In most states, you can still recover damages reduced by your percentage of fault. Some states don\'t allow recovery if you\'re over 50% at fault. Consult an attorney to understand your state\'s rules.' },
                { q: 'How much should I expect to settle for?', a: 'Settlements range from $15,000 for minor injuries to $2,000,000+ for catastrophic injuries. Most truck accident cases settle between $100,000-$500,000 depending on injury severity and liability.' },
                { q: 'What if the truck driver was uninsured or underinsured?', a: 'You can recover from your own uninsured/underinsured motorist (UM/UIM) coverage. Many truck companies carry $1,000,000+ in insurance, but if limits are exceeded, your UM/UIM coverage applies.' },
                { q: 'Can I still sue if I signed a settlement waiver?', a: 'It depends on when you signed it and under what circumstances. If you signed before fully understanding your injuries, you may be able to challenge it. Never sign any documents without attorney review.' },
                { q: 'What is the difference between workers comp and personal injury?', a: 'Workers comp covers workplace injuries with limited benefits and no pain & suffering damages. Personal injury lawsuits allow full damages including pain & suffering, lost earning capacity, and future medical care. If not work-related, file personal injury.' }
              ].map((item, idx) => (
                <Card key={idx} style={{ padding: '16px', backgroundColor: 'white', borderRadius: '4px', borderLeft: '3px solid #c4714a' }}>
                  <div style={{ fontSize: '14px', fontWeight: '600', color: '#1a4a5a', marginBottom: '8px' }}>
                    {item.q}
                  </div>
          ...
          {/* Related Arti          <div style={{ marginBottom: '56px' }} id="immediate-actions">
            <h2 style={{ color: '#1a4a5a', fontSize: '31px', fontWeight: '700', marginBottom: '20px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
          ...
          <div style={{ marginBottom: '56px' }}>
            ...
            <p style={{ color: '#555', lineHeight: '1.8', marginBottom: '24px', fontSize: '21px' }}>
              Explore other personal injury guides to understand your legal options and settlement expectations across different accident types.
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: isMobileView ? '1fr' : '2fr', gap: '16px' }}>
              {[
                { title: 'Car Accident Guide', description: 'Liability, insurance claims, and settlement expectations after a motor vehicle collision', link: '/guide/car-accident' },
                { title: 'Slip & Fall Guide', description: 'Property owner liability, premises liability laws, and how to prove negligence', link: '/guide/slip-and-fall' },
                { title: 'Medical Malpractice Guide', description: 'Standard of care, expert testimony requirements, and complex damages', link: '/guide/medical-malpractice' },
                { title: 'Motorcycle Accident Guide', description: 'Bias against riders, insurance tactics, and catastrophic injury settlements', link: '/guide/motorcycle-accident' }
              ].map((guide, idx) => (
                <a key={idx} href={guide.link} style={{ textDecoration: 'none' }}>
                  <Card style={{ padding: '20px', backgroundColor: 'white', borderLeft: '4px solid #4a8c7e', borderRadius: '6px', transition: 'all 0.3s ease', cursor: 'pointer' }} onMouseEnter={(e) => e.currentTarget.style.boxShadow = '0 8px 24px rgba(0,0,0,0.08)'} onMouseLeave={(e) => e.currentTarget.style.boxShadow = 'none'}>
                    <h3 style={{ color: '#1a4a5a', fontSize: '16px', fontWeight: '700', margin: 0, marginBottom: '8px' }}>
                      {guide.title}
                    </h3>
                    <p style={{ color: '#555', fontSize: '14px', lineHeight: '1.6', margin: '8px 0 0 0' }}>
                      {guide.description}
                    </p>
                    <p style={{ color: '#4a8c7e', fontSize: '13px', fontWeight: '600', margin: '12px 0 0 0' }}>
                      Learn more →
                    </p>
                  </Card>
                </a>
        ...
        {!isMobileView && (
          <div style={{ width: '280px', padding: '40px 0', position: 'sticky', top: '80px', height: 'fit-content' }}>
            <div style={{ fontSize: '12px', fontWeight: '700', color: '#1a4a5a', marginBottom: '16px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              On This Page
            </div>
