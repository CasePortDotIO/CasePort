...
import { InsertUser, users, applications, InsertApplication, waitlist, InsertWaitlist } from "../drizzle/schema";
...
export async function addToWaitlist(data: InsertWaitlist): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
