  ...
  for (const article of articles) {
    ...
    if (!categoryId) {
      ...
      if (existingCat.docs.length > 0) {
        ...
        const newCat = await payload.create({
          collection: 'categories',
          data: { title: titleParam, slug: cleanSlug(titleParam) },
        })
    ...
    const authorName =
      typeof article.author === 'object'
        ? article.author.name
        : article.author || 'CasePort Editorial'
    ...
    if (!authorId) {
      const newAuthor = await payload.create({
        ...
        data: {
          name: authorName,
          title: 'Contributor',
          bio: 'A contributor on case acquisition and personal injury marketing strategy.',
        },
      })
        ...
        children: [
          {
            ...
            mode: 'normal',
            style: '',
            text: article.excerpt || 'Lorem ipsum...',
            version: 1,
          },
        ],
      })
