        ...
        <div className="calc-card" style={{ maxWidth: "40rem", margin: "0 auto" }}>
          ...
          <div className="th-result" id="thResult">
            ...
            <div className="th-bar">
              <div className="th-bar-seg net" style={{ width: seg(net) + "%" }} title={`Your net: ${fmt(net)}`}></div>
              <div className="th-bar-seg fee" style={{ width: seg(feeAmt) + "%" }} title={`Attorney fee: ${fmt(feeAmt)}`}></div>
              <div className="th-bar-seg costs" style={{ width: seg(costs) + "%" }} title={`Case costs: ${fmt(costs)}`}></div>
              <div className="th-bar-seg liens" style={{ width: seg(liens) + "%" }} title={`Liens: ${fmt(liens)}`}></div>
            </div>
          ...
          <p className="est-note" style={{ marginTop: "1rem" }}>
            Illustrative only. Fees, costs, and liens vary by case and state — and liens are
            often negotiable, which can raise your net. This is general information, not a quote
            or legal advice.
          </p>
        </div>
        <div className="th-links" id="thLinks">
          <Link href="/guide/how-contingency-fees-work" className="card-link">
            How the attorney fee works <Icon name="arrow" />
          </Link>
          <Link href="/guide/medical-liens-subrogation" className="card-link">
            How liens are negotiated down <Icon name="arrow" />
          </Link>
        </div>
      </div>
    </section>
