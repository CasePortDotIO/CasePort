...
function Panel({ regionId }: { regionId: string | null }) {
  ...
  if (!region) {
    return (
      <div className="empty">
        <Icon name="steth" />
        <p style={{ fontSize: "1.05rem", fontWeight: 700, color: "var(--text-4)" }}>Select an area</p>
        ...
        return (
          <Link key={slug} href={`/injuries/${slug}`} className="sm-injury">
            ...
            <span className="card-link" style={{ marginTop: ".6rem", fontSize: ".85rem" }}>
              See full guide <Icon name="arrow" />
            </span>
          </Link>
        );
      })}
    ...
    <section className="section bg-white" data-widget="symptomMatcher">
      <div className="container">
        ...
        <div className="sm-layout">
          <div className="sm-picker">
            <div className="sm-bodywrap">
              <svg className="sm-figure" viewBox="0 0 360 560" role="img" aria-label="Body region selector">
        ...
        <p className="note" style={{ justifyContent: "center", marginTop: "1.5rem" }}>
          <Icon name="alertC" />
          <span>
            This is educational guidance, not a diagnosis. If you have emergency symptoms, call
            911 or go to the ER.
          </span>
        </p>
      </div>
