...
import { article, medicalWebPage, faqSchema, breadcrumb, speakable, orgGraph } from "@/lib/schema";
import { guidePillar, guidePillars, accidentTypes, guides, guideSpokeDefs, hasGuideSpokes, injuries, medReviewer } from "@/data";
import { reviewer } from "@/lib/constants";
import { guideScene, guidePillarLead } from "@/lib/guide";
import { toSections, dashLabel, readingMinutes } from "@/lib/article";

const INJ_BRIDGE: Record<string, string> = {
  "car-accident": "whiplash", "truck-accident": "traumatic-brain-injury", "motorcycle-accident": "broken-bones",
  "pedestrian-accident": "traumatic-brain-injury", "bicycle-accident": "broken-bones", "rideshare-accident": "whiplash",
  "slip-and-fall": "back-injury", "workplace-injury": "back-injury", "wrongful-death": "traumatic-brain-injury",
...
export function GuidePillarPage({ slug }: { slug: string }) {
  ...
  const minutes = readingMinutes(lead, prose.flatMap((s) => s.paras));
  const takeaways = keyFacts.slice(0, 4).map(dashLabel);

  // cross-hub bridges
  const bridges: [string, string, string, string][] = [];
  if (p.kind === "accident" && at) {
    bridges.push([`/accidents/${slug}`, "scale", `${name} claims by state`, `How your state’s negligence rule and deadline affect a ${name.toLowerCase()} claim.`]);
  }
  const injB = INJ_BRIDGE[slug];
  if (injB && injuries[injB]) {
    bridges.push([`/injuries/${injB}`, "steth", `Common ${name.toLowerCase()} injuries`, "Symptoms, treatment, and recovery for injuries seen in these cases."]);
  }
  if (p.kind === "accident") {
    bridges.push(["/guide/dealing-with-insurance", "shield", "Dealing with insurance", "What the adjuster is really doing — and what to say."]);
    bridges.push(["/guide/how-to-document-an-accident", "camera", "Document your accident", "The exact evidence to capture before it disappears."]);
  } else {
    bridges.push(["/guide/faq", "list", "Injury claim FAQ", "Direct answers to the most common claim questions."]);
    bridges.push(["/accidents/first-hour", "clock", "The first hour after a crash", "The immediate steps that protect a claim."]);
  }
  ...
  return (
    <>
      <HeroPhoto
        ...
        scene={p.scene}
        img={guideScene(slug)}
        byline={<Byline reviewerName={isMedical ? medReviewer.name : reviewer.name} isMedical={isMedical} minutes={minutes} onDark />}
      />

      {stats.length > 0 && (
        <StatTiles category="National guide" stats={stats} />
      )}

      <KeyTakeaways items={takeaways} />

      <Capsule heading={`What you need to know about ${(at ? at.category : name).toLowerCase()}`} lead={lead} />
      ...
      <Sources medical={isMedical} citeTitle={`${name} — The Complete Guide`} citeUrl={`/guide/${slug}`} />

      <CTABand title={`Questions About Your ${name} Claim?`} sub="Get a free, confidential case review — no cost, no obligation." btn="Get Free Case Review" />
      ...
      <JsonLd
        data={[
          isMedical
            ? medicalWebPage({ headline: `${name} — The Complete Guide`, description: directAnswer.slice(0, 180) })
            : article({ headline: `${name} — The Complete Guide`, description: directAnswer.slice(0, 180) }),
          faqSchema(faqs),
          breadcrumb([
            { name: "Home", url: "/" },
