...
import { article, faqSchema, breadcrumb, speakable, orgGraph } from "@/lib/schema";
import { NATIONAL, negColor, cityData, stateLawTopics, stateLawFor } from "@/data";
  ...
  return (
    <>
      <section className="page-intro">
        <div className="container-5">
          ...
          <p className="section-sub" style={{ marginTop: "1rem" }}>
            {s.name} applies {s.label}. Here is the negligence rule, filing deadline,
            damage caps, and local data that determine your recovery.
          </p>
      ...
      <section style={{ padding: "0 1.5rem 2.5rem" }}>
        <div className="container-5">
          <div className="card r" style={{ borderColor: color + "40", borderLeft: "3px solid " + color }}>
      ...
      <KeyTakeaways items={["Negligence rule", "Filing deadline", "Fault threshold", "Damage cap"]} />
        ...
        <section className="section bg-cream">
          <div className="container-5">
            ...
            <div className="city-col" style={{ maxWidth: "46rem" }}>
              ...
              >
                {cd.cities.map((c) => (
                  <Link key={c.slug} href={`/accidents/${r.cityKey}/${c.slug}`} className="city-link">
                    {c.name}{" "}
          ...
          <div className="container-5">
            <div className="card r center" style={{ maxWidth: "40rem", margin: "0 auto" }}>
              <p style={{ fontSize: "1.05rem", color: "var(--text-2)" }}>
                City-level money pages for {s.name} are rolling out now. Full statewide
                legal coverage is live above. See a finished example of a city guide:
              </p>
              <Link
              ...
              >
                See a live city page <Icon name="arrow" />
              </Link>
            </div>
          </div>
        </section>
      ...
      <FAQ faqs={sFaqs} bg="bg-white" container="container-4" title={`Frequently Asked Questions — ${s.name}`} />

      <Expert bg="bg-cream" />

      <Sources citeTitle={`Accident Law in ${s.name} — Negligence, Deadlines & Caps`} citeUrl={`/accidents/${r.slug}`} />
